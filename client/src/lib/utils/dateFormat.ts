 ...
 */
export function formatDate(value: DateTime | Date | string | undefined | null): string {
  if (!value) return "";
  ...
  return fromJS.isValid ? fromJS.toLocaleString(DateTime.DATE_MED) : String(value);
}

/**
 * Format a date-like value to include both date and time.
 */
export function formatDateTime(value: DateTime | Date | string | undefined | null): string {
  if (!value) return "";
  ...
  const fromJS = DateTime.fromJSDate(new Date(String(value)));
  return fromJS.isValid ? fromJS.toLocaleString(DateTime.DATETIME_MED) : String(value);
}
