...
function PodiumSlot({
  ...
  return <div className="flex flex-col items-center">{slotInner}</div>;
}
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((e) => (
                <div key={e.id} className={rowCard}>
                  ...
                  {e.sub && (
                    <span className={`text-[10px] ${subtextColor} shrink-0 opacity-50`}>
                      {e.sub}
                    </span>
                  )}
                </div>
          ...
          {tabs.map((tab) => {
            ...
            const inactiveClass = useLiquidGlass
              ...
              : useDarkMode
                ? "hover:bg-gray-700"
                : "hover:bg-gray-100";
            const btnClass = `relative z-10 flex flex-1 cursor-pointer items-center justify-center gap-1.5 rounded-xl px-3 py-2 text-xs font-semibold transition-all duration-200 ${isActive ? activeClass : inactiveClass}`;
            const btnContent = (
              <span
                  ...
                  <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
                    {statChips.map((chip, i) => (
                      <div key={i} className={`flex flex-col gap-1 rounded-xl p-3 ${panelBg}`}>
                        ...
                        {chip.sub && (
                          <span className={`text-[10px] ${subtextColor} opacity-60`}>
                            {chip.sub}
                          </span>
                        )}
                      </div>
