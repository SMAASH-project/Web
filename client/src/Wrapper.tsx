...
import { getAverageHexColor, getTextColor, lightenHexColor, toRgbaColor } from "./lib/utils";
...
export function Wrapper({ children }: WrapperProps) {
  ...
  const { currentGradient, cssVars } = useMemo(() => {
    ...
    const themeAccent = lightenHexColor(themeAverage, settings.useDarkMode ? 0.08 : 0.02);
    const themeAccentHover = lightenHexColor(themeAverage, settings.useDarkMode ? 0.22 : 0.14);
    const themeAccentSoft = toRgbaColor(themeAverage, settings.useDarkMode ? 0.32 : 0.25);
  ...
  } else if (settings.animationOverride !== "none" && settings.animationOverride !== "custom") {
    resolvedAnimation = settings.animationOverride as AnimationKey;
  }

  const effectMix = context?.effectMix ?? null;
  const useComposite =
    settings.animationOverride === "custom" && effectMix !== null && hasEnabledEffects(effectMix);
